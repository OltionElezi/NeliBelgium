...
const symbolRenderers: Record<string, (doc: PDFKit.PDFDocument, x: number, y: number, size: number, props: any) => void> = {
  // Outlets
...
const defaultRenderer = (doc: PDFKit.PDFDocument, x: number, y: number, size: number) => {
...
export const generateElectricalPdf = async (
...
function drawCoverPage(
  ...
  // Company logo
  const logoPath = path.join(process.cwd(), 'src', 'image-removebg-preview (1).png');
...
function drawDiagramPage(
  ...
  const diagramData = diagram.diagramData as unknown as DiagramData;
    ...
    if (diagramData.connections) {
      ...
      for (const conn of diagramData.connections) {
        if (conn.points && conn.points.length >= 4) {
          doc.moveTo(
            offsetX + conn.points[0] * scale,
            offsetY + conn.points[1] * scale
          );
          for (let i = 2; i < conn.points.length; i += 2) {
            doc.lineTo(
              offsetX + conn.points[i] * scale,
              offsetY + conn.points[i + 1] * scale
            );
          }
          if (conn.dashed) {
            doc.dash(5, { space: 3 });
